...
class Rankings {
	...
	private function _guildsRanking() {
		...
		switch(mconfig('guild_score_formula')) {
			...
			case 3:
				...
				break;
			default:
				$result = $this->mu->query_fetch("SELECT TOP ".$this->_results." Name, EmblemUrl, Wins, Losses, TotalPoint FROM Clan WHERE DeleteFlag=0  ORDER BY Point DESC");
		}
	...
	private function _masterlevelRanking() { //master
		$this->mu = Connection::Database('MuOnline');
		
		if(_TBL_CHR_ == _TBL_MASTERLVL_) {
			// Master Level and Character in same table
			//$result = $this->mu->query_fetch("SELECT TOP ".$this->_results." Name, Level, LC, BP, XP FROM "._TBL_CHR_." WHERE DeleteFlag = 0 AND Name NOT IN(".$this->_rankingsExcludeChars().") AND Level > 0 ORDER BY LC DESC");
			$result = $this->mu->query_fetch("SELECT TOP ".$this->_results." t1.Name, t1.Level, t1.LC, t1.BP, t1.XP, t1.CID, t2.CID, t2.Ranking, t2.Wins, t2.Losses, t2.Score FROM "._TBL_CHR_." AS t1 INNER JOIN BZCharacterInfo AS t2 ON t1.CID = t2.CID WHERE t1.DeleteFlag = 0 AND t1.Name NOT IN(".$this->_rankingsExcludeChars().") AND t2.Ranking > 0 ORDER BY t2.Score DESC");
		} else {
			// Master Level in separate table
			$result = $this->mu->query_fetch("SELECT TOP ".$this->_results." t1.CID, t1.Ranking, t1.Wins, t1.Losses, t1.Score FROM "._TBL_MASTERLVL_." AS t1 INNER JOIN "._TBL_CHR_." AS t2 ON t1.Name = t2.Name WHERE t1.Name NOT IN(".$this->_rankingsExcludeChars().") AND t1.Level > 0 ORDER BY t1.Level DESC, t2.Level DESC");
		}
		if(!is_array($result)) return;
	...
	private function _getLevelRankingData($combineMasterLevel=false) {
		...
		if(!$combineMasterLevel) {
			$result = $this->mu->query_fetch("SELECT TOP ".$this->_results." Name, Level, BP, XP, Sex FROM "._TBL_CHR_." WHERE DeleteFlag = 0 AND Name NOT IN(".$this->_rankingsExcludeChars().") AND Level > 0 ORDER BY Level DESC");
			if(!is_array($result)) return;
			return $result;
		}
		...
		if(_TBL_CHR_ == _TBL_MASTERLVL_) {
			...
			$result = $this->mu->query_fetch("SELECT TOP ".$this->_results." Name, Level, LC, BP, XP, Sex FROM "._TBL_CHR_." WHERE DeleteFlag = 0 AND Name NOT IN(".$this->_rankingsExcludeChars().") AND Level > 0 ORDER BY Level DESC");
			if(!is_array($result)) return;
			return $result;
		} else {
	...
	private function _getKillersRankingData($combineMasterLevel=false) {
		...
		// level only (no master level)
		if(!$combineMasterLevel) {
			$result = $this->mu->query_fetch("SELECT TOP ".$this->_results." Name, Level, KillCount, Sex FROM "._TBL_CHR_." WHERE DeleteFlag = 0 AND Name NOT IN(".$this->_rankingsExcludeChars().") AND Level > 0 ORDER BY KillCount DESC");
			if(!is_array($result)) return;
			return $result;
		}
		...
		if(_TBL_CHR_ == _TBL_MASTERLVL_) {
			// level + master level (in same table)
			$result = $this->mu->query_fetch("SELECT TOP ".$this->_results." Name, Level, KillCount FROM "._TBL_CHR_." WHERE DeleteFlag = 0 AND Name NOT IN(".$this->_rankingsExcludeChars().") AND Level > 0 ORDER BY KillCount DESC");
			if(!is_array($result)) return;
			return $result;
		} else {
			// level + master level (different tables)
